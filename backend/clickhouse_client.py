...
# A tiny key/value table that SURVIVES container rebuilds (data lives in the
# clickhouse_data volume). Both backend and ml-engine read/write it, so the
# first page load right after `docker compose up --build` is instant — it serves
# the last computed snapshot instead of recomputing heavy aggregations cold.
CACHE_TABLE = f"{CLICKHOUSE_DB}.dashboard_cache"


def ensure_cache_table() -> bool:
    """Create the persistent cache table if missing. Idempotent; safe every boot."""
    client = get_client()
    if not client:
        return False
    try:
...
def cache_set(key: str, value) -> bool:
    """Store any JSON-serialisable value (dict or list) under key. Latest wins."""
    client = get_client()
    if not client:
        return False
    try:
...
def cache_get(key: str):
    """Return (value, age_seconds) for key, or None if absent/error."""
    try:
        rows = _q(
